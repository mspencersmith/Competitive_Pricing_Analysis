...
class WebScraper:
    ...
    def aspects(self, filename, pages=None, write=False):
        ...
        with open(filename, mode, newline='') as csv_file:
            ...
            for item in soup.select('.property-box-inner'):
                try:
                    ...
                    accommodation = 'Aspects property'
    ...
    def airbnb(self, filename, offset=None, write=False):
        ...
        if write:
            ...
            url = f'{url}&items_offset={offset}&section_offset=3'
            mode = 'a'
        response, soup = self._get_response(url)
        with open(filename, mode, newline='') as csv_file:
            csv_writer = csv.writer(csv_file)
        ...
        with open(filename, mode, newline='') as csv_file:
            ...
            for item in soup.select('.sr_property_block'):
                ...
                try:
                    ...
                    for i in item.select('.c-unit-configuration__item'):
    ...
    def _get_response(self, url):
        response = requests.get(url, headers=WebScraper.headers)
        soup = BeautifulSoup(response.content, 'lxml')
        return response, soup

f = 'data/2021.12.25.csv'
print(f'Writing to {f}..')

ws = WebScraper()
ws.booking(f, write=True)
print('\nAppending booking.com data')
for i in tqdm(range(25, 51, 25)):
